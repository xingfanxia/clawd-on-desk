...
インストール後に Clawd を起動してください。**新規インストールはデフォルトで「シンプルモード」** — Clawd はピュアなデスクトップペットになり、agent hook 自動登録も AI 脳もオンボーディングウィザードもありません。AI 機能（agent hook 自動同期 + 魂の脳 + チャット + 画面観察 + 日記 + 永続メモリ）を有効にするには、`Settings…` → `General` → `AI Features` をオンにして Clawd を再起動してください。すでに AI 機能を使っていた既存ユーザー（オンボーディング完了済み、または `~/.clawd/{soul.json,config.json}` がディスク上に存在）はアップグレード時にマイグレーションロジックが advanced モードを自動的に維持します。
...
# Clawd を起動（デフォルトはシンプルモード。Settings で AI Features を有効化後に再起動）
npm start
```

`AI Features` を有効にすると、**Claude Code** と **Codex CLI** は自動登録される hook により、そのまま動作します。**Gemini CLI**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI (Kimi-CLI)**、**opencode** は、インストール済みであれば Clawd 起動時に自動登録されます。**Copilot CLI** だけは一度だけ hook setup が必要です。Remote SSH、WSL、macOS/Linux のプラットフォーム別メモも含めた詳細: **[docs/guides/setup-guide.md](docs/guides/setup-guide.md)**
...
<details>
<summary>コントリビューター 41 人をすべて表示</summary>
