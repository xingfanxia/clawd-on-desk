...
설치 후 Clawd를 실행하세요. **새로 설치하면 기본적으로 "Simple Mode"로 시작합니다** — Clawd는 순수 데스크톱 펫이며 agent hook 자동 등록도, AI 두뇌도, 온보딩 마법사도 없습니다. AI 기능(agent hook 자동 동기화 + 영혼 두뇌 + 채팅 + 화면 관찰 + 일기 + 지속 메모리)을 켜려면 `Settings…` → `General` → `AI Features` 토글을 켠 다음 Clawd를 재시작하세요. 이미 AI 기능을 사용 중이던 기존 사용자(온보딩 완료 또는 `~/.clawd/{soul.json,config.json}` 파일 존재)는 업그레이드 시 마이그레이션 휴리스틱이 고급 모드를 자동으로 유지합니다.
...
# Clawd 시작 (기본 Simple Mode; Settings에서 AI 기능을 켠 후 재시작)
npm start
```

`AI Features`를 켜면 **Claude Code**와 **Codex CLI**는 바로 사용할 수 있습니다. **Gemini CLI**, **Cursor Agent**, **CodeBuddy**, **Kiro CLI**, **Kimi Code CLI (Kimi-CLI)**, **opencode**는 설치되어 있다면 Clawd 시작 시 자동 등록되며, **Copilot CLI**만 1회 hook 설정이 필요합니다. 원격 SSH, WSL, 플랫폼별 참고 사항(macOS / Linux)까지 포함된 가이드는 **[docs/guides/setup-guide.md](docs/guides/setup-guide.md)** 를 참고하세요.
