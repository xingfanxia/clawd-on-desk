...
安装后启动 Clawd。**全新安装默认进入"简洁模式"** —— Clawd 只是一个纯桌面宠物：不连 agent hooks、不连 AI 大脑、不弹引导。要打开 AI 功能（agent hook 自动同步 + 灵魂大脑 + 对话 + 屏幕观察 + 日记 + 持久记忆），打开 `Settings…` → `General` → `AI Features` 切换开关后重启 Clawd。已经接入过 AI 功能的老用户（完成过 onboarding，或本机 `~/.clawd/{soul.json,config.json}` 存在）升级时会自动保留高级模式 —— 迁移逻辑会保护你的现有配置。
...
# 启动 Clawd（默认简洁模式；在 Settings 中开启 AI 功能后重启生效）
npm start
```

开启 `AI Features` 后，**Claude Code** 和 **Codex CLI** 会自动注册 hooks，开箱即用。**Gemini CLI**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI（Kimi-CLI）**、**opencode** 在已安装前提下会在 Clawd 启动时自动同步；**Copilot CLI** 仍需一次性手动配置 hooks。也涵盖远程 SSH、WSL 及平台说明（macOS / Linux）：**[docs/guides/setup-guide.zh-CN.md](docs/guides/setup-guide.zh-CN.md)**
